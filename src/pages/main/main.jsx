...
import { LoginModal, SignupModal, ProjcetUploadModal } from "../../components/modals";
...
const Main = () => {
  ...
  const getProject = async () => {
	try {
        const project = await axios.get(`http://localhost:2008/apis/project`);
  ...
  return (
    <Container>
      <ButtonWrap>
        <Button label="새 프로젝트 등록" onClick={handleProjectUploadOpen}/>
      </ButtonWrap>
      <ListWrapper>
        <List>
			{projects?.map((item) => (
            <Item onClick={() => handleToProjectDetail(item._id)}>{item.title}</Item>
          ))}
        </List>
      </ListWrapper>
	  <LoginModal
	  ...
	  <ProjcetUploadModal isOpen={isProjectUploadOpen} onClose={handleProjectUploadClose} getProject={getProject}/>
    </Container>
  );
};
